...
package net.mcreator.migem.keybind;
...
import net.mcreator.migem.procedures.DieOnKeyPressedProcedure;
...
public class DieKeyBinding extends MigemModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public DieKeyBinding(MigemModElements instance) {
		super(instance, 4);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MigemMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
