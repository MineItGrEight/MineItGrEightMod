...
package eu.aldin.mirge.mirgem.keybind;
...
import eu.aldin.mirge.mirgem.procedures.DieOnKeyPressedProcedure;
import eu.aldin.mirge.mirgem.MirgemModElements;
import eu.aldin.mirge.mirgem.MirgemMod;

@MirgemModElements.ModElement.Tag
public class DieKeyBinding extends MirgemModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public DieKeyBinding(MirgemModElements instance) {
		super(instance, 4);
	...
	public void initElements() {
		keys = new KeyBinding("key.mirgem.die", GLFW.GLFW_KEY_I, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MirgemMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				DieOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
